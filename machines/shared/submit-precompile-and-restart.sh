...
while getopts "haf:g:m:n:p:oq:r:st:u:w" opt; do
  case $opt in
    h)
      echo "Precompile the system image(s) and then restart a simulation. Arguments are passed through to submit-restart.sh:"
      echo
      ./submit-restart.sh -h
      exit 1
      ;;
...
./submit-restart.sh -f $MK_SYSIMAGE_JOB_ID $POSTPROC_ARGUMENT -w $@
