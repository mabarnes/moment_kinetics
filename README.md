...
2) Create a subdirectory to store run output.
    ...
    $ mkdir runs
    ```
3) To pre-compile a static image (`dependencies.so`) that includes a few of the external packages required for post-processing, run
    ```
...
4) To run julia with optimization, type
    ```
    $julia -O3 --project -Jdependencies.so run_moment_kinetics.jl
    ```
    ...
    ```
    $ julia -O3 --project -Jdependencies.so run_moment_kinetics.jl input.toml
    ```
...
5) To make plots and calculate frequencies/growth rates, run
    ```
    $ julia --project -Jdependencies.so run_post_processing.jl runs/<directory to process>
    ```
...
4b) Parameter scans or performance tests can be performed by running
    ```
    $ julia -O3 --project -Jdependencies.so driver.jl
    ```
...
5b) Post processing can be done for several directories at once using
    ```
    $ julia --project -Jdependencies.so post_processing_driver.jl runs/<directory1> runs/<directory2> ...
    ```
...
* Run using `Pkg`. Either using `pkg>` mode
    ```
    $ julia -O3 --project -Jdependencies.so
    ...
    ```
    julia -O3 --project -J dependencies.so -e "import Pkg; Pkg.test()`
    ```
* Execute some or all of the tests as a script. For example in the terminal run
    ```
    $ julia -O3 --project -Jdependencies.jl test/runtests.jl
    ```
    or in the REPL run
    ```
