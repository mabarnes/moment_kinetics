...
if abspath(PROGRAM_FILE) == @__FILE__
	...
	vpa_nelement_local = 16 # number of elements per rank
	...
	vperp_nelement_local = 16 # number of elements per rank
    ...
    if test_Rosenbluth_integrals
        ...
        println("max(dHdvpa_err)",maximum(dHdvpa_err))
        println("max(dHdvperp_err)",maximum(dHdvperp_err))
        println("max(d2Gdvperp2_err)",maximum(d2Gdvperp2_err))
        println("max(d2Gdvpa2_err)",maximum(d2Gdvpa2_err))
        println("max(d2Gdvperpdvpa_err)",maximum(d2Gdvperpdvpa_err))
