#!/bin/bash

# Run the memory_profile.jl script with memory profiling activated
# First argument to the script gives the input file to use
#../bin/julia -O3 --check-bounds=no --project --track-allocation=user memory_profile.jl $1
../bin/julia -O3 --check-bounds=no --project --track-allocation=all memory_profile.jl $1

echo "done profiling, collecting results"
echo ""

# Print top 5 allocation sites for convenience
../bin/julia --project collect_memory_stats.jl
