...
using moment_kinetics.type_definitions: OptionsDict
    ...
    function gausslegendre_test(; ngrid=17, nelement=4, L_in=6.0)
        ...
        for y_name in ["vpa","vperp","z"]
            ...
            f_exact = allocate_float(y.n)
            f_num = allocate_float(y.n)
            f_err = allocate_float(y.n)
            df_exact = allocate_float(y.n)
            df_num = allocate_float(y.n)
            df_err = allocate_float(y.n)
            g_exact = allocate_float(y.n)
            h_exact = allocate_float(y.n)
            h_num = allocate_float(y.n)
            h_err = allocate_float(y.n)
            divg_exact = allocate_float(y.n)
            divg_num = allocate_float(y.n)
            divg_err = allocate_float(y.n)
            laph_exact = allocate_float(y.n)
            laph_num = allocate_float(y.n)
            laph_err = allocate_float(y.n)
            d2f_exact = allocate_float(y.n)
            d2f_num = allocate_float(y.n)
            d2f_err = allocate_float(y.n)
            fperiodic_err = allocate_float(y.n)
            fperiodic_num = allocate_float(y.n)
            fperiodic_exact = allocate_float(y.n)
            d2fperiodic_exact = allocate_float(y.n)
            d2fperiodic_num = allocate_float(y.n)
            d2fperiodic_err = allocate_float(y.n)
            b = allocate_float(y.n)
            for iy in 1:y.n
