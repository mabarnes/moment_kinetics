...
using Printf
using ArbNumerics: sqrt, elliptic_k, ArbFloat, exp, log
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    integrand = Array{ArbFloat,1}(undef,ngrid)
    integrand_sqrty = Array{ArbFloat,1}(undef,ngrid)
    integrand_Kz = Array{ArbFloat,1}(undef,ngrid)
    value_Kz = Array{ArbFloat,1}(undef,ngrid)
    integrand_Kz_sqrt = Array{ArbFloat,1}(undef,ngrid)
    value_Kz_sqrt = Array{ArbFloat,1}(undef,ngrid)
    integrand_Kz_sqrt_diff = Array{ArbFloat,1}(undef,ngrid)
    value_Kz_sqrt_diff = Array{ArbFloat,1}(undef,ngrid)
    value_y = Array{ArbFloat,1}(undef,ngrid)
    value_z = Array{ArbFloat,1}(undef,ngrid)
    
    xx = Array{ArbFloat,1}(undef,ngrid)
    ww = Array{ArbFloat,1}(undef,ngrid)
    @. xx = ArbFloat(x)
    @. ww = ArbFloat(w)
    ...
    for i in 1:ngrid
        ...
        integrand[i] = sqrt(1.0/y)*ww[i]
        integrand_sqrty[i] = sqrt(y)*ww[i]
        z = (ArbFloat(1.0) - y)
        #z = (1.0 - y)/(1.0 + 10^-15)
        #ellipk_z = ellipk(z)
        ellipk_z = elliptic_k(z)
        #if isnan(ellipk_z) || isinf(ellipk_z)
        #    ellipk_z = 0.0
        #end
        sqrt_1_z = sqrt(ArbFloat(1.0) - z)
        #if isinf(sqrt_1_z) || isinf(sqrt_1_z)
        #    sqrt_1_z = 1.0
        #end
        integrand_Kz[i] = ellipk_z*ww[i]
        value_Kz[i] = ellipk_z
        integrand_Kz_sqrt[i] = ellipk_z*ww[i]/sqrt_1_z
        value_Kz_sqrt[i] = ellipk_z/sqrt_1_z
        value_Kz_sqrt_diff[i] = (ellipk_z - log(4.0) + log(sqrt_1_z))/sqrt_1_z
        integrand_Kz_sqrt_diff[i] = value_Kz_sqrt_diff[i]*ww[i]
        value_z[i] = z
    end
    #@. integrand *= w
