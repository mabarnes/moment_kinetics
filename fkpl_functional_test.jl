...
using moment_kinetics.gauss_legendre: gausslegendre_mass_matrix_solve!
...
using moment_kinetics.fokker_planck_test: dHdvpa_Maxwellian, dHdvperp_Maxwellian, Cssp_Maxwellian_inputs
...
using moment_kinetics.initial_conditions: create_boundary_distributions, create_pdf
...
function test_Lagrange_Rosenbluth_potentials(ngrid,nelement; standalone=true)
    ...
    vr_input = grid_input("vr", 1, 1, 1, 
        1, 0, 1.0, "", "", "", "", adv_input,MPI.COMM_NULL,"uniform")
    ...
    vzeta, vzeta_spectral = define_coordinate(vzeta_input)
    r, r_spectral = define_coordinate(r_input)
    z, z_spectral = define_coordinate(z_input)
    composition = species_composition(n_ion_species, n_ion_species, n_neutral_species,
    boltzmann_electron_response, false, 1.0, 1.0,
    1.0, 0.0, 1.0, 0.0, allocate_float(n_ion_species))
    nuii = 1.0
    collisions = collisions_input(0.0, 0.0, false, 0.0, "", false, nuii, 0.0, "none")
    rk_coefs = setup_runge_kutta_coefficients(4)
    advance = advance_info(false, false, false, false, false, false,
    ...
    pdf_unused = create_pdf(composition, r, z, vperp, vpa, vzeta, vr, vz)
...
function test_strong_form_collision_operator(;ngrid=5,nelement_list=[2],plot_scan=true)
...
if abspath(PROGRAM_FILE) == @__FILE__
    using Pkg
    Pkg.activate(".")
    test_strong_form_collision_operator()
end 
