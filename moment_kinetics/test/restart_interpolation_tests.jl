...
function run_test(test_input, base, message, rtol, atol; tol_3V, args...)
    ...
    if global_rank[] == 0
        ...
        @test isapprox((1/3) .* expected.ppar_ion[:, end], newgrid_p_ion[:,1], rtol=rtol)
        ...
        # The errors on p_neutral when using a 3V grid are large - probably because of
        # linear interpolation in ion-neutral interaction operators - so for the 3V tests
        # we have to use a very loose tolerance for p_neutral.
        newgrid_p_neutral = interpolate_to_grid_z(expected.z, p_neutral[:, :, end], z, z_spectral)
        @test isapprox((1/3) .* expected.ppar_neutral[:, end], newgrid_p_neutral[:,1], rtol=rtol_3V)
