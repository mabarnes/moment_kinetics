...
function gyroaverage_test(absolute_error; rhostar=0.1, pitch=0.5, ngrid=5, kr=2, kz=2, phaser=0.0, phasez=0.0, nelement=4, ngrid_vperp=3, nelement_vperp=1, Lvperp=3.0, ngrid_gyrophase=100, discretization="chebyshev_pseudospectral", r_bc="periodic", z_bc = "wall", print_test_results=print_test_results, test_output_directory)
        ...
        r, r_spectral = define_coordinate(r_input, nothing; collision_operator_dim=false, run_directory=test_output_directory)
        z, z_spectral = define_coordinate(z_input, nothing; collision_operator_dim=false, run_directory=test_output_directory)
        vperp, vperp_spectral = define_coordinate(vperp_input, nothing; collision_operator_dim=false, run_directory=test_output_directory)
        vpa, vpa_spectral = define_coordinate(vpa_input, nothing; collision_operator_dim=false, run_directory=test_output_directory)
        gyrophase, gyrophase_spectral = define_coordinate(gyrophase_input, nothing; collision_operator_dim=false, run_directory=test_output_directory)
