...
# When we need to halo-swap multiple variables at the same time, it will be more efficient
# to only wait twice, rather than twice for each variable.
function halo_swap!(x_list::AbstractArray...; r, z)
    if block_rank[] == 0
        reqs_r = MPI.Request[]
        for x ∈ x_list
