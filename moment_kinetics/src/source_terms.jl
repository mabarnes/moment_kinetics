...
function source_terms_evolve_ppar_no_collisions!(pdf_out, pdf_in, dens, upar, ppar, vth,
    ...
    vpa_grid = vpa.grid
    @loop_r_z ir iz begin
        factor1 = dt * (-ddens_dz[iz,ir] * vth[iz,ir] / dens[iz,ir] + dvth_dz[iz,ir])
        factor2 = -dt * 0.5 * dqpar_dz[iz,ir] / ppar[iz,ir]

        @loop_vperp_vpa ivperp ivpa begin
            pdf_out[ivpa,ivperp,iz,ir] += pdf_in[ivpa,ivperp,iz,ir] *
                                          (factor1 * vpa_grid[ivpa] + factor2)
        end
    end
