...
@timeit global_timer source_terms!(
    ...
    if moments.evolve_ppar
        ...
        @loop_s_r_z is ir iz begin
            coefficient = dn_dt[iz,ir,is] / n[iz,ir,is] - dvth_dt[iz,ir,is] / vth[iz,ir,is]
            @loop_vperp_vpa ivperp ivpa begin
                pdf_out[ivpa,ivperp,iz,ir,is] += dt * coefficient * pdf_in[ivpa,ivperp,iz,ir,is]
            end
        end
    elseif moments.evolve_density
        @loop_s_r_z is ir iz begin
            coefficient = dn_dt[iz,ir,is] / n[iz,ir,is]
            @loop_vperp_vpa ivperp ivpa begin
                pdf_out[ivpa,ivperp,iz,ir,is] += dt * coefficient * pdf_in[ivpa,ivperp,iz,ir,is]
            end
        end
...
@timeit global_timer source_terms_neutral!(
    ...
    if moments.evolve_ppar
        ...
        @loop_sn_r_z isn ir iz begin
            coefficient = dn_dt[iz,ir,isn] / n[iz,ir,isn] - dvth_dt[iz,ir,isn] / vth[iz,ir,isn]
            @loop_vzeta_vr_vz ivzeta ivr ivz begin
                pdf_out[ivz,ivr,ivzeta,iz,ir,isn] += dt * coefficient * pdf_in[ivz,ivr,ivzeta,iz,ir,isn]
            end
        end
    elseif moments.evolve_density
        @loop_sn_r_z isn ir iz begin
            coefficient = dn_dt[iz,ir,isn] / n[iz,ir,isn]
            @loop_vzeta_vr_vz ivzeta ivr ivz begin
                pdf_out[ivz,ivr,ivzeta,iz,ir,isn] += dt * coefficient * pdf_in[ivz,ivr,ivzeta,iz,ir,isn]
            end
        end
