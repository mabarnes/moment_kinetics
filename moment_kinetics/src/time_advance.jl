...
function setup_time_info(t_input, n_variables, code_time, dt_reload,
    ...
    if is_electron
        ...
        dt = fill(mk_float(dt_reload === nothing ? mk_float(t_input["dt"]) : dt_reload), r.n)
        previous_dt = copy(dt)
        dt_before_output = copy(dt)
        dt_before_last_fail = fill(mk_float(dt_before_last_fail_reload === nothing ? mk_float(Inf) : dt_before_last_fail_reload), r.n)
