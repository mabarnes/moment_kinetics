...
function init_fokker_planck_collisions_weak_form(vpa,vperp,vpa_spectral,vperp_spectral; precompute_weights=false, test_dense_matrix_construction=false, print_to_screen=true)
    ...
    S_dummy = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    Q_dummy = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    rhsvpavperp = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    rhsvpavperp_copy1 = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    rhsvpavperp_copy2 = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    rhsvpavperp_copy3 = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    
    CC = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    GG = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    HH = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    dHdvpa = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    dHdvperp = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    dGdvperp = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    d2Gdvperp2 = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    d2Gdvpa2 = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    d2Gdvperpdvpa = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    
    FF = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    ...
    rhs_advection = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    # dummy arrays for JFNK
    Fnew = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    Fresidual = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    F_delta_x = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    F_rhs_delta = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    Fv = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    Fw = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
...
function allocate_fokkerplanck_arrays_direct_integration(vperp,vpa)
    ...
    G0_weights = allocate_shared_float(vpa, vperp, vpa, vperp)
    G1_weights = allocate_shared_float(vpa, vperp, vpa, vperp)
    H0_weights = allocate_shared_float(vpa, vperp, vpa, vperp)
    H1_weights = allocate_shared_float(vpa, vperp, vpa, vperp)
    H2_weights = allocate_shared_float(vpa, vperp, vpa, vperp)
    H3_weights = allocate_shared_float(vpa, vperp, vpa, vperp)
    ...
    dHdvpa = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    dHdvperp = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    #Cflux_vpa = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    #Cflux_vperp = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    buffer_vpavperp_1 = allocate_float(vpa, vperp)
    buffer_vpavperp_2 = allocate_float(vpa, vperp)
    Cssp_result_vpavperp = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    dfdvpa = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    d2fdvpa2 = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    d2fdvperpdvpa = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    dfdvperp = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
    d2fdvperp2 = allocate_shared_float(vpa, vperp; comm=comm_anysv_subblock[])
...
function setup_fp_nl_solve(implicit_ion_fp_collisions::Bool,
    ...
    fp_solver_section = set_defaults_and_check_section!(
        input_dict, "fokker_planck_collisions_nonlinear_solver", warn_unexpected;
        rtol=0.0,
        atol=1.0e-10,
