...
function runtests()
    @testset "calculus" verbose=use_verbose begin
        ...
        @testset "finite_difference derivatives (4 argument), periodic" verbose=false begin
            @testset "$nelement $ngrid $deriv_order" for nelement ∈ (1:5), ngrid ∈ (9:33),
                                            deriv_order ∈ (Val(1), Val(2))
                ...
                if deriv_order == Val(1)
                    expected_df = @. 2.0 * π / L * cospi(2.0 * x.grid / L)
                    rtol = 1.e2 / (nelement*(ngrid-1))^4
                elseif deriv_order == Val(2)
                    expected_df = @. -(2.0 * π / L)^2 * sinpi(2.0 * x.grid / L)
                    rtol = 1.e2 / (nelement*(ngrid-1))^2
                else
                    error("deriv_order=$deriv_order is not implemented")
                end

                # differentiate f
                derivative!(df, f, x, fd_info(), deriv_order)
        ...
        @testset "finite_difference derivatives upwinding (5 argument), periodic" verbose=false begin
            @testset "$fd_option $order $nelement $ngrid" for
                    (fd_option, order) ∈ (
        ...
        @testset "finite_difference derivatives (4 argument), Neumann" verbose=false begin
            @testset "$bc $nelement $ngrid $deriv_order" for bc ∈ ("constant", "zero"),
                    nelement ∈ (1:5), ngrid ∈ (9:33), deriv_order ∈ (Val(1), Val(2))

                if bc == "constant" && deriv_order == Val(2)
                    # Skip as this option does not converge correctly at the boundary
                    # points
                    continue
                end
        ...
        @testset "finite_difference derivatives upwinding (5 argument), Neumann" verbose=false begin
            @testset "$bc $fd_option $nelement $ngrid" for bc ∈ ("constant", "zero"),
        ...
        @testset "$cheb_implementation derivatives (4 argument), periodic" verbose=false for
                cheb_implementation ∈ ("chebyshev_pseudospectral",
                                       "chebyshev_pseudospectral_matrix_multiply")
            @testset "$nelement $ngrid $deriv_order" for (nelement, ngrid, deriv_order, rtol) ∈
                    (
                     (1, 5,  Val(1), 8.e-1),
                     (1, 5,  Val(2), 8.e-1),
                     (1, 6,  Val(1), 2.e-1),
                ...
                if cheb_implementation == "chebyshev_pseudospectral" && deriv_order == Val(2)
                    # Second derivatives not implemented for FFT-based Chebyshev method
                    continue
                end
                ...
                # differentiate f
                derivative!(df, f, x, spectral, deriv_order)

                @test isapprox(df, expected_df, rtol=rtol, atol=atol,
                               norm=maxabs_norm)
            end
        end
        ...
        @testset "$cheb_implementation derivatives (4 argument), Neumann" verbose=false for
                cheb_implementation ∈ ("chebyshev_pseudospectral",
                                       "chebyshev_pseudospectral_matrix_multiply")
            @testset "$nelement $ngrid $deriv_order" for bc ∈ ("constant", "zero"),
                    nelement ∈ (1:5), ngrid ∈ (3:33), deriv_order ∈ (Val(1), Val(2))
                ...
                for n ∈ 0:ngrid-1
                    ...
                    elseif deriv_order == Val(2)
                        for p ∈ 2:n
                            coefficient = randn(rng)
                            @. f += coefficient * x.grid ^ p
                            @. expected_df += coefficient * p * (p - 1) * x.grid ^ (p - 2)
                        end
                    else
                        error("deriv_order=$deriv_order is not implemented")
                    end
        ...
        @testset "$cheb_implementation derivatives upwinding (5 argument), Neumann" verbose=false for
                cheb_implementation ∈ ("chebyshev_pseudospectral",
                                       "chebyshev_pseudospectral_matrix_multiply")
            @testset "$bc $nelement $ngrid" for bc ∈ ("constant", "zero"),
