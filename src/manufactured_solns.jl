...
using IfElse
    ...
    function upari_sym(Lr,Lz,r_bc,z_bc,composition,geometry,nr)
        if z_bc == "periodic"
    ...
    function ppari_sym(Lr,Lz,r_bc,z_bc,composition)
        if z_bc == "periodic"
    ...
    function manufactured_sources(r_coord,z_coord,vperp_coord,vpa_coord,vzeta_coord,vr_coord,vz_coord,composition,geometry,collisions,num_diss_params)
        ...
        upari = upari_sym(r_coord.L,z_coord.L,r_coord.bc,z_coord.bc,composition,geometry,r_coord.n)
        vthi = vthi_sym(r_coord.L,z_coord.L,r_coord.bc,z_coord.bc,composition,vperp_coord.n)
