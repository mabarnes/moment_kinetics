...
function create_pdf(composition, r, z, vperp, vpa, vzeta, vr, vz)
    ...
    pdf_charged_unnorm = allocate_shared_float(vpa.n, vperp.n, z.n, r.n, composition.n_ion_species)
    ...
    if use_manufactured_solns
        ...
        if n_neutral_species > 0
            update_neutral_qz!(moments.neutral.qz, moments.neutral.qz_updated,
                               ...
                               moments.neutral.vth, pdf.neutral.unnorm, vz, vr, vzeta, z,
            ...
            update_neutral_pz!(moments.neutral.pz, moments.neutral.pz_updated,
                               moments.neutral.dens, moments.neutral.uz,
                               pdf.neutral.unnorm, vz, vr, vzeta, z, r, composition,
...
function initialize_pdf!(pdf, moments, boundary_distributions, composition, r, z, vperp,
    ...
    @serial_region begin
        for is ∈ 1:composition.n_ion_species, ir ∈ 1:r.n
            ...
            pdf.charged.unnorm[:,:,:,ir,is] .= pdf.charged.norm[:,:,:,ir,is]
            @loop_z iz begin
                @. pdf.charged.unnorm[:,:,iz,ir,is] *= moments.charged.dens[iz,ir,is]
                if moments.evolve_ppar
                    @. pdf.charged.norm[:,:,iz,ir,is] *= moments.charged.vth[iz,ir,is]
                elseif moments.evolve_density == false
                    @. pdf.charged.norm[:,:,iz,ir,is] = pdf.charged.unnorm[:,:,iz,ir,is]
                end
            end
        end
    end
    ...
    @serial_region begin
        @loop_sn_r isn ir begin
            ...
            pdf.neutral.unnorm[:,:,:,:,ir,isn] .= pdf.neutral.norm[:,:,:,:,ir,isn]
            @loop_z iz begin
                @. pdf.neutral.unnorm[:,:,:,iz,ir,isn] *= moments.neutral.dens[iz,ir,isn]
                if moments.evolve_ppar
                    @. pdf.neutral.norm[:,:,:,iz,ir,isn] *= moments.neutral.vth[iz,ir,isn]
                elseif moments.evolve_density == false
                    @. pdf.neutral.norm[:,:,:,iz,ir,isn] = pdf.neutral.unnorm[:,:,:,iz,ir,isn]
                end
            end
        end
    end
...
function enforce_neutral_wall_bc!(pdf, z, vzeta, vr, vz, pz, uz, density, wall_flux_0,
    ...
    if !evolve_upar
        ...
        if z.irank == 0
            ...
            @views wall_flux_0 += pdf_norm_fac * integrate_over_negative_vz(abs.(vz.scratch2) .* pdf[:,:,:,1], vz.scratch2, vz.wgts, vz.scratch, vr.grid, vr.wgts, vzeta.grid, vzeta.wgts)
            ...
            @loop_vz ivz begin
                if vz.scratch2[ivz] >= -zero
                    @views @. pdf[ivz,:,:,1] = wall_flux_0 * knudsen_cosine[ivz,:,:] * pdf_norm_fac
                end
            end
        end
        ...
        if z.irank == z.nrank - 1
            ...
            # add this species' contribution to the combined ion/neutral particle flux out of the domain at z=-Lz/2
            @views wall_flux_L += pdf_norm_fac * integrate_over_positive_vz(abs.(vz.scratch2) .* pdf[:,:,:,end], vz.scratch2, vz.wgts, vz.scratch, vr.grid, vr.wgts, vzeta.grid, vzeta.wgts)
            ...
            @loop_vz ivz begin
                if vz.scratch2[ivz] <= zero
                    @views @. pdf[ivz,:,:,end] = wall_flux_L * knudsen_cosine[ivz,:,:] * pdf_norm_fac
                end
            end
        end
    else
...
function convert_full_f_neutral_to_normalised!(f, density, uz, pz, vth, vzeta, vr, vz,
    ...
    @loop_z iz begin
        # Calculate moments
        @views density[iz] = integrate_over_vspace(f[:,:,:,iz], vz.grid, 0, vz.wgts,
                                                   vr.grid, 0, vr.wgts, vzeta.grid, 0,
                                                   vzeta.wgts)
        @views uz[iz] = integrate_over_vspace(f[:,:,:,iz], vz.grid, 1, vz.wgts, vr.grid,
                                              0, vr.wgts, vzeta.grid, 0, vzeta.wgts) /
                             density[iz]
        @views pz[iz] = integrate_over_vspace(f[:,:,:,iz], vz.grid, 2, vz.wgts, vr.grid,
                                              0, vr.wgts, vzeta.grid, 0, vzeta.wgts) -
                             density[iz]*uz[iz]^2
