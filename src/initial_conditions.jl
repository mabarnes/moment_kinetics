...
function init_pdf_over_density!(pdf, spec, vpa, z, vth, vpa_norm_fac)
    ...
    @outerloop for iz ∈ 1:z.n
        ...
        @. scratch = vpa.grid^2 * pdf[:,iz] * (vpa_norm_fac[iz]/vth[iz])^2
        pparfac = integrate_over_vspace(scratch, vpa.wgts)
        # pparfac2 = the integral of the pdf over v-space, weighted by m_s w_s^2 (w_s^2 - vths^2 / 2) / vth^4
        #@. scratch = vpa.grid^2 *(vpa.grid^2/pparfac - vth[iz]^2/densfac) * pdf[iz,:] * (vpa_norm_fac[iz]/vth[iz])^4
        @. scratch = vpa.grid^2 *(vpa.grid^2/pparfac - 1.0/densfac) * pdf[:,iz] * (vpa_norm_fac[iz]/vth[iz])^4
        pparfac2 = integrate_over_vspace(scratch, vpa.wgts)
