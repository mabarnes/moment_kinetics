            ...
            for ir ∈ 1:r.n
                ...
                for is ∈ 1:composition.n_ion_species
                        ...
                        @views wall_flux_0 += (sqrt(composition.mn_over_mi) *
                                               integrate_over_negative_vpa(abs.(vz_charged[:,:]) .* f_charged[:,:,1,ir,is], vz_charged[:,1], 0, vpa.wgts, vpa.scratch, vpa.ngrid, vperp.grid, 0, vperp.wgts))
                        # get velocity into the wall at this r at z = L/2, vz(vpa,vperp)
                        vz_charged = z_adv_charged[is].speed[end,:,:,ir]
                        @views wall_flux_L += (sqrt(composition.mn_over_mi) *
                                               integrate_over_positive_vpa(abs.(vz_charged[:,:]) .* f_charged[:,:,end,ir,is], vz_charged[:,1], 0, vpa.wgts, vpa.scratch, vpa.ngrid, vperp.grid, 0, vperp.wgts))
                end
