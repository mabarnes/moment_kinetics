...
struct scratch_pdf
    pdf::MPISharedArray{pdf_dims_tuple, mk_float, pdf_ndims}
    density::MPISharedArray{moment_dims_tuple, mk_float, moment_ndims}
    upar::MPISharedArray{moment_dims_tuple, mk_float, moment_ndims}
    ppar::MPISharedArray{moment_dims_tuple, mk_float, moment_ndims}
    temp_z_s::MPISharedArray{moment_dims_tuple, mk_float, moment_ndims}
end
...
function setup_time_advance!(pdf, vpa, z, composition, drive_input, moments,
    ...
    z_SL = setup_semi_lagrange(Val(:z), z.n, phase_space_dims; vpa=vpa.n)
    vpa_SL = setup_semi_lagrange(Val(:vpa), vpa.n, phase_space_dims; z=z.n)
...
function setup_scratch_arrays(moments, pdf_in, n_rk_stages)
    ...
    pdf_dim_sizes = NamedTuple{pdf_dims_tuple}(size(pdf_in))
    moment_dim_sizes = NamedTuple{moment_dims_tuple}(size(moments.dens))
    ...
    for istage ∈ 1:n_rk_stages+1
        ...
        pdf_array = allocate_shared_float(pdf_dims; pdf_dim_sizes...)
        density_array = allocate_shared_float(moment_dims; moment_dim_sizes...)
        upar_array = allocate_shared_float(moment_dims; moment_dim_sizes...)
        ppar_array = allocate_shared_float(moment_dims; moment_dim_sizes...)
        temp_z_s_array = allocate_shared_float(moment_dims; moment_dim_sizes...)
...
function setup_runge_kutta_coefficients(n_rk_stages)
    rk_coefs = allocate_float(Val((:term,:rk_stage)); term=3, rk_stage=n_rk_stages)
    rk_coefs .= 0.0
    if n_rk_stages == 4
        rk_coefs[1,1] = 0.5
        rk_coefs[3,1] = 0.5
...
function time_advance!(pdf, scratch, t, t_input, vpa, z, vpa_spectral, z_spectral,
    ...
    for i ∈ 1:t_input.nstep
        ...
        if mod(i,t_input.nwrite) == 0
            block_synchronize()
            block_rank[] == 0 && println("finished time step ", i)
...
# scratch should be a (nz,nspecies) array
function update_pdf_unnorm!(pdf, moments, scratch, composition, z)
